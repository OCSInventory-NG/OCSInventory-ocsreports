...
class LinfoOutput {
	...
	*/
	...
	public function htmlOut() {
		...
		if (isset($settings['theme']) && strpos($settings['theme'], '..') === false && file_exists('libraries/linfo/layout/theme_'.$settings['theme'].'.css'))
			$theme_css = 'theme_'.$settings['theme'].'.css';

		// Does default exist?? Don't bitch at me for assigning an array key in an if-then
		elseif (($settings['theme'] = 'default') && file_exists('libraries/linfo/layout/theme_'.$settings['theme'].'.css'))
			$theme_css = 'theme_'.$settings['theme'].'.css';
