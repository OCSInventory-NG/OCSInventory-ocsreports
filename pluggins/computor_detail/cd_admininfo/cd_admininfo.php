...
$chiffres="onKeyPress=\"return scanTouche(event,/[0-9]/)\" onkeydown='convertToUpper(this)'
	...
	if (isset($ESC_POST['Valid_modif_x'])){
		if ($ESC_POST['TAG_MODIF'] == TAG_LBL)
		$lbl_champ='TAG';
		else
		$lbl_champ=$ESC_POST['TAG_MODIF'];
		$sql=" update accountinfo set ".$lbl_champ."='";
		if ($ESC_POST['FIELD_FORMAT'] == "date")
		$sql.= dateToMysql($ESC_POST['NEW_VALUE'])."'";
		else
		$sql.= xml_encode($ESC_POST['NEW_VALUE'])."'";
	...
	if (isset($ESC_POST['MODIF']) and $ESC_POST['MODIF'] != ''){
		switch ($type_field[$ESC_POST['MODIF']]){
		...
		$truename=$ESC_POST['MODIF'];
		if ($ESC_POST['MODIF'] == TAG_LBL)
			$truename='TAG';			
		if ($type_field[$ESC_POST['MODIF']]=="date"){
		...
		$data_form[0]=$ESC_POST['MODIF'];
		tab_modif_values($data_form,$tab_typ_champ,array('TAG_MODIF'=>$ESC_POST['MODIF'],'FIELD_FORMAT'=>$type_field[$ESC_POST['MODIF']]),$l->g(895),"");
