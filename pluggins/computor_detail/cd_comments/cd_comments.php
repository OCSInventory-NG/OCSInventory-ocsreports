<?php 
	...
	if ($ESC_POST['del_check'] != ''){
			$sql="update itmgmt_comments set visible=0 where id in (".$ESC_POST['del_check'].")";
			mysql_query($sql, $_SESSION["writeServer"]);
		 	//regénération du cache
			$tab_options['CACHE']='RESET';	 	
	 }	

	if ($ESC_POST['SUP_PROF'] != '' and isset($ESC_POST['SUP_PROF'])){
		$sql="update itmgmt_comments set visible=0 where id=".$ESC_POST['SUP_PROF'];
		mysql_query($sql, $_SESSION["writeServer"]);
		//regénération du cache
		$tab_options['CACHE']='RESET';
		addLog($l->g(896), " DEL => ".$ESC_POST['SUP_PROF']);

	}
	
	if ($ESC_POST['Valid_modif_x'] != '' and isset($ESC_POST['Valid_modif_x'])){
		
		//ajout de note
		if (trim($ESC_POST['NOTE']) != '' and isset($ESC_POST['NOTE'])){
			$sql="insert into itmgmt_comments (HARDWARE_ID,DATE_INSERT,USER_INSERT,COMMENTS,ACTION) 
					value (".$systemid.",sysdate(),'".$_SESSION["loggeduser"]."','".xml_encode($ESC_POST['NOTE'])."','ADD_NOTE_BY_USER')";
			mysql_query($sql, $_SESSION["writeServer"]) or die(mysql_error($_SESSION["readServer"]));
			//regénération du cache
			$tab_options['CACHE']='RESET';			
		}elseif (trim($ESC_POST['NOTE_MODIF']) != '' and isset($ESC_POST['NOTE_MODIF'])){
			$sql="update itmgmt_comments set COMMENTS='".xml_encode($ESC_POST['NOTE_MODIF'])."'";
			if (!strstr($ESC_POST['USER_INSERT'], $_SESSION["loggeduser"]))
			$sql.=" , USER_INSERT = '".$ESC_POST['USER_INSERT']."/".$_SESSION["loggeduser"]."'";
			$sql.=" where id=".$ESC_POST['ID_MODIF'];
			mysql_query($sql, $_SESSION["writeServer"]) or die(mysql_error($_SESSION["readServer"]));
			//regénération du cache
			$tab_options['CACHE']='RESET';				
			addLog($l->g(896), " UPDATE ".$ESC_POST['ID_MODIF'].". => ".$ESC_POST['OLD_COMMENTS'] );

		}		
		
	}
	if ($ESC_POST['ADD_NOTE']){
		$tab_name[1]=$l->g(897)." : ";
		$tab_name[2]=$l->g(898)." : ";
		$tab_name[3]=$l->g(896)." : ";
	...
	if (isset($ESC_POST['MODIF']) and $ESC_POST['MODIF'] != ''){
		$queryDetails = "SELECT ID,DATE_INSERT,USER_INSERT,COMMENTS,ACTION FROM itmgmt_comments WHERE id=".$ESC_POST['MODIF'];
		...
		$tab_hidden['ID_MODIF']=$ESC_POST['MODIF'];
