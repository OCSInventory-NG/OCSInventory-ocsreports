...
function show_ligne($value,$color,$id_field,$ajout,$form_name){
	...
	if ($ajout != ''){
		...
		$and_or .= "<option value='AND' ".($ESC_POST['SelAndOr-'.$nameField] == "AND" ? " selected":"")." >AND</option>";
		$and_or .= "<option value='OR' ".($ESC_POST['SelAndOr-'.$nameField] == "OR" ? " selected":"")." >OR</option>";
		$and_or .= "</select>";
	...
	$select="<select name='SelComp-".$nameField."' id='SelComp-".$nameField."'>";
		foreach ($champ_select as $k=>$v){
			...
			if ($k!='javascript'){
				...
				$select .= "<option value='".$k."' ".($ESC_POST['SelComp-'.$nameField] == $k ? " selected":"")." ".$champ_select['javascript'][$k].">".$v."</option>";
			}
		}										
	$select .= "</select>";
	...
	if( array_key_exists($value,$optSelectField)){		
		echo "<td>".$select."&nbsp;&nbsp;<input name='InputValue-".$nameField."' id='InputValue-".$nameField."' value=\"".stripslashes($ESC_POST["InputValue-".$nameField])."\">&nbsp;";
	...
	if( array_key_exists($value,$opt2SelectField) or array_key_exists($value,$optSelect)){
		...
		if (is_array($data[$value.'-SQL1'])){
			foreach ($data[$value.'-SQL1'] as $k=>$v){	
				$select2 .= "<option value='".$k."' ".($ESC_POST[$name_select."-".$nameField] == $k ? " selected":"").">".$v."</option>";
			}
		}else{
			...
			while( $val = mysql_fetch_array( $result ) ) {
				foreach ($val as $name_of_field=>$value_of_request){
					if (!is_numeric($name_of_field) and $name_of_field != 'ID'){
						...
						$select2 .= "<option value='".$val['ID']."' ".($ESC_POST[$name_select.'-'.$nameField] == $val['ID'] ? " selected":"").">".$value_of_request."</option>";
					}
				}
		...
		if (array_key_exists($value,$opt2SelectField)){
			...
			echo $select."&nbsp;&nbsp;<input name='InputValue-".$nameField."' id='InputValue-".$nameField."' value=\"".stripslashes($ESC_POST["InputValue-".$nameField])."\">&nbsp;".$opt2SelectField[$value."-LBL"];
		}
		echo "</td></tr>";
	}
	...
	if( array_key_exists($value,$opt2Select)){
		...
		if (is_array($opt2Select[$value.'-SQL1'])){
			foreach ($opt2Select[$value.'-SQL1'] as $k=>$v){		
				$selectValue .= "<option value='".$k."' ".($ESC_POST['SelFieldValue-'.$nameField] == $k ? " selected":"").">".$v."</option>";
			}
		}else{
			...
			while( $val = mysql_fetch_array( $result ) ) {
				if (!isset($val['ID']))
				$val['ID']=$val['NAME'];
					$selectValue .= "<option value='".$val['ID']."' ".($ESC_POST['SelFieldValue-'.$nameField] == $val['ID'] ? " selected":"").">".$val['NAME']."</option>";
			}
		}
		$selectValue .= "</select>";
	...
	if( array_key_exists($value,$optSelect2Field)){
		...
		//on fonction du POST
		if ($ESC_POST['SelComp-'.$nameField] == "between")
		$display="inline";
		else
		$display="none";
		
		echo "<td>".$select."&nbsp;&nbsp;<input name='InputValue-".$nameField."' id='InputValue-".$nameField."' value=\"".stripslashes($ESC_POST["InputValue-".$nameField])."\">
				 <div style='display:".$display."' id='FieldInput2-".$nameField."'>&nbsp;--&nbsp;<input name='InputValue2-".$nameField."' value=\"".stripslashes($ESC_POST["InputValue2-".$nameField])."\"></div>".$optSelect2Field[$value."-LBL"]."</td></tr>";
	}
	...
	if( array_key_exists($value,$opt3Select)){
		...
		while( $val = mysql_fetch_array( $result ) ) {
			if (!isset($val['ID']))
			$val['ID']=$val['NAME'];
				$selectValue1 .= "<option value='".$val['ID']."' ".($ESC_POST['SelFieldValue-'.$nameField] == $val['ID'] ? " selected":"").">".$val['NAME']."</option>";
		}
		$selectValue1 .= "</select>";
		...
		while( $val = mysql_fetch_array( $result ) ) {
			if (!isset($val['ID']))
			$val['ID']=$val['NAME'];
				$selectValue2 .= "<option value='".$val['ID']."' ".($ESC_POST['SelFieldValue2-'.$nameField] == $val['ID'] ? " selected":"").">".$val['NAME']."</option>";
		}
		$selectValue2 .= "</select>";
