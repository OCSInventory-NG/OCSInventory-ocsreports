 ...
 * $input_name = nom du champ que l'on va r�cup�rer en $ESC_POST
 ...
 * $name = nom du champ
 * $input_name = nom du champ r�cup�r� dans le $ESC_POST
 * $input_type = 0 : <input type='text'>
 * 				 1 : <textarea>
 * 				 2 : <select><option>
...
function filtre($tab_field,$form_name,$query){
	global $ESC_POST,$l;
	if ($ESC_POST['RAZ_FILTRE'] == "RAZ")
	unset($ESC_POST['FILTRE_VALUE'],$ESC_POST['FILTRE']);
	if ($ESC_POST['FILTRE_VALUE'] and $ESC_POST['FILTRE']){
...
//			echo "<br>";
		...
		$temp_query[0].= " where ";
	$query=$temp_query[0].$ESC_POST['FILTRE']." like '%".$ESC_POST['FILTRE_VALUE']."%' ";
	...
	$view.=show_modif(stripslashes($ESC_POST['FILTRE_VALUE']),'FILTRE_VALUE',0);
...
function nb_page($form_name,$taille_cadre='80',$bgcolor='#C7D9F5',$bordercolor='#9894B5'){
	...
	foreach( $machNmb as $nbm ) {
		$pcParPageHtml .=  "<option".($ESC_POST["pcparpage"] == $nbm ? " selected" : "").($countHl%2==1?" class='hi'":"").">$nbm</option>";
		$countHl++;
	}
	...
	echo $pcParPageHtml;
	if (isset($ESC_POST["pcparpage"])){
		$deb_limit=$ESC_POST['page']*$ESC_POST["pcparpage"];
		$fin_limit=$deb_limit+$ESC_POST["pcparpage"]-1;		
	}
	
//	echo $deb_limit."=>".$fin_limit."<br>";
	//echo "deb => ".$deb_limit."    fin => ".$fin_limit."<br>";
	echo "<input type='hidden' id='SHOW' name='SHOW' value='".$ESC_POST['SHOW']."'>";
	//return (array("BEGIN"=>$ESC_POST["pcparpage"],"END"=>$deb_limit));
	return (array("BEGIN"=>$deb_limit,"END"=>$fin_limit));
}

function show_page($valCount,$form_name){
	global $ESC_POST;
	if (isset($ESC_POST["pcparpage"]) and $ESC_POST["pcparpage"] != 0)
	$nbpage= ceil($valCount/$ESC_POST["pcparpage"]);
	if ($nbpage >1){
	$up=$ESC_POST['page']+1;
	$down=$ESC_POST['page']-1;
	...
	if ($ESC_POST['page']< $nbpage-1)
	echo "<img src='image/proch24.png' OnClick='pag(\"".$up."\",\"page\",\"".$form_name."\")'>";
	
	}
	echo "</td></tr></table>";
	echo "<input type='hidden' id='page' name='page' value='".$ESC_POST['page']."'>";
	echo "<input type='hidden' id='old_pcparpage' name='old_pcparpage' value='".$ESC_POST['pcparpage']."'>";
}
...
{
	global $ESC_POST;
	$ESC_POST['onglet_soft']=stripslashes($ESC_POST['onglet_soft']);
	$ESC_POST['old_onglet_soft']=stripslashes($ESC_POST['old_onglet_soft']);
	if ($ESC_POST["old_".$post_name] != $ESC_POST[$post_name]){
	$ESC_POST['page']=0;
	}
	...
	$i=0;
	  foreach($def_onglets as $key=>$value){
	  	...
	  	echo "<li ";
	  	if (is_numeric($ESC_POST[$post_name])){
			if ($ESC_POST[$post_name] == $key or (!isset($ESC_POST[$post_name]) and $current != 1)){
			 echo "id='current'";  
	 		 $current=1;
			}
	  	}else{
	  		//echo "<script>alert('".mysql_escape_string(stripslashes($ESC_POST[$post_name]))." => ".$key."')</script>";
			if (mysql_escape_string(stripslashes($ESC_POST[$post_name])) === mysql_escape_string(stripslashes($key)) or (!isset($ESC_POST[$post_name]) and $current != 1)){
				 echo "id='current'";  
	 			 $current=1;
			}
		}
	...
	echo "<input type='hidden' id='".$post_name."' name='".$post_name."' value='".$ESC_POST[$post_name]."'>";
	echo "<input type='hidden' id='old_".$post_name."' name='old_".$post_name."' value='".$ESC_POST[$post_name]."'>";
	}
...
function gestion_col($entete,$data,$list_col_cant_del,$form_name,$tab_name,$list_fields,$default_fields,$id_form='form'){
	...
	if (isset($ESC_POST['SUP_COL']) and $ESC_POST['SUP_COL'] != ""){
		unset($_SESSION['col_tab'][$tab_name][$ESC_POST['SUP_COL']]);
	}
	if ($ESC_POST['restCol'.$tab_name]){
	...
	$select_restCol .= "<option".($ESC_POST["restCol".$tab_name] == "DEFAULT" ? " selected" : "")." value=''>".$l->g(32)."...</option>";
	$countHl=0;
	if ($list_rest != ""){
		// ksort($list_rest,SORT_STRING);
		// print_r($list_rest);
		foreach( $list_rest as $lbl_field => $name_field) {
			$select_restCol .=  "<option".($ESC_POST["restCol".$tab_name] == $lbl_field ? " selected" : "").($countHl%2==0?" class='hi'":"")." value='".$lbl_field."'>$name_field</option>";
			$countHl++;
		}
	}
...
function tab_req($table_name,$list_fields,$default_fields,$list_col_cant_del,$queryDetails,$form_name,$width='100',$tab_options='')
{
	...
	if ($ESC_POST['tri2'] == "" or (!in_array ($ESC_POST['tri2'], $list_fields) and !in_array ($ESC_POST['tri2'], $tab_options['AS'])))
	$ESC_POST['tri2']=1;
//	echo $ESC_POST['tri2'];
	
	//si le tri se fait sur un champ fixe
	if ( strrpos($ESC_POST['tri2'], ".") and ((substr($ESC_POST['tri2'],1,1) != '.' and $ESC_POST['tri2']!=1)
		 or ($ESC_POST['tri_fixe']!= '' and $ESC_POST['tri2'] == ""))){
		//echo "<b>".$ESC_POST['tri2']."</b>";
		if ($ESC_POST['tri_fixe'] !=  $ESC_POST['tri2'])
		$ESC_POST['tri_fixe']=$ESC_POST['tri2'];
		//on veut savoir sur quelle table le tri doit avoir lieu
		$tablename_fixe_value=explode(".", $ESC_POST['tri_fixe']);			
		$ESC_POST['tri2']=1;		
	}
//echo $ESC_POST['tri2'];
	if ($ESC_POST['sens'] == "")
	...
	if ($tab_options['TRI']['SIGNED'][$ESC_POST['tri2']])
		$queryDetails.= " order by cast(".$ESC_POST['tri2']." as signed) ".$ESC_POST['sens'];
	else
		$queryDetails.= " order by ".$ESC_POST['tri2']." ".$ESC_POST['sens'];
...
//		$limit["END"]=$_SESSION['NUM_ROW'][$table_name];
	...
	if ($tab_options['CACHE']=='RESET' or (isset($ESC_POST['SUP_PROF']) and $ESC_POST['SUP_PROF'] != '') ){
...
//echo $limit["END"];
//		$value_data_begin=$ESC_POST['page']*$ESC_POST['pcparpage'];
//		$value_data_end=$value_data_begin+$ESC_POST['pcparpage'];
	...
	}else
	{
		...
		if (isset($_SESSION['SQL_DATA_FIXE'][$table_name])){
			foreach ($_SESSION['SQL_DATA_FIXE'][$table_name] as $key=>$sql){
				...
				if ($ESC_POST['tri_fixe']!='' and strstr($sql,$ESC_POST['tri_fixe']))
				$sql.=" order by ".$ESC_POST['tri_fixe']." ".$ESC_POST['sens'];
			//	$sql.=" limit 200";
...
//		$data=$data_limit;
	...
	//echo "<br>tri=".$ESC_POST['tri2'];
		...
		echo "<input type='hidden' id='tri_fixe' name='tri_fixe' value='".$ESC_POST['tri_fixe']."'>";
...
//					echo "<br>".$value;
			...
			//	echo $no_alias_value."<br>";
				...
				//	echo $key."<br>";
					...
					elseif ($key == "CHECK"){
						$data[$i][$num_col]="<input type='checkbox' name='check".$value_of_field."' id='check".$value_of_field."' ".$javascript." ".(isset($ESC_POST['check'.$value_of_field])? " checked ": "").">";
