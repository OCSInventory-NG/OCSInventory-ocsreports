	...
	if ($rule_exist == 'NAME_NOT_EXIST'){
		...
		while ($RULE_VALUES['PRIORITE_'.$i]){
			if ($RULE_VALUES['CFIELD_'.$i] != "")
			{
				...
				value (".$ID_RULE.",'".$ESC_POST['RULE_NAME']."',".$RULE_VALUES['PRIORITE_'.$i].",'".$RULE_VALUES['CFIELD_'.$i]."','".$RULE_VALUES['OP_'.$i]."','".$RULE_VALUES['COMPTO_'.$i]."','".$RULE_VALUES['COMPTO_TEXT_'.$i]."')";
...
function fields_conditions_rules($num,$entete='NO'){
	...
	if (!isset($ESC_POST["PRIORITE_".$num]))
	$ESC_POST["PRIORITE_".$num]=$num;
	$tab.="<tr><td>".show_modif($ESC_POST["PRIORITE_".$num],"PRIORITE_".$num,'0')."</td>";
