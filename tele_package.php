...
//print_r($ESC_POST);
if( isset( $ESC_POST["VALID_END"] ) ) {
	$sql_details=array('document_root'=>$ESC_POST['document_root'],
					   'timestamp'=>$ESC_POST['timestamp'],
					   'nbfrags'=>$ESC_POST["nbfrags"],
					   'name'=>$ESC_POST['NAME'],
					   'os'=>$ESC_POST['OS'],
					   'description'=>$ESC_POST['DESCRIPTION'],
					   'size'=>$ESC_POST['SIZE']);
					   
	$info_details=array('PRI'=>$ESC_POST['PRIORITY'],
						'ACT'=>$ESC_POST['ACTION'],
						'DIGEST'=>$ESC_POST['digest'],
						'PROTO'=>$ESC_POST['PROTOCOLE'],
						'DIGEST_ALGO'=>$ESC_POST["digest_algo"],
						'DIGEST_ENCODE'=>$ESC_POST["digest_encod"],
						'PATH'=>$ESC_POST['ACTION_INPUT'],
						'NAME'=>$ESC_POST['ACTION_INPUT'],
						'COMMAND'=>$ESC_POST['ACTION_INPUT'],
						'NOTIFY_USER'=>$ESC_POST['NOTIFY_USER'],
						'NOTIFY_TEXT'=>$ESC_POST['NOTIFY_TEXT'],
						'NOTIFY_COUNTDOWN'=>$ESC_POST['NOTIFY_COUNTDOWN'],
						'NOTIFY_CAN_ABORT'=>$ESC_POST['NOTIFY_CAN_ABORT'],
						'NOTIFY_CAN_DELAY'=>$ESC_POST['NOTIFY_CAN_DELAY'],
						'NEED_DONE_ACTION'=>$ESC_POST['NEED_DONE_ACTION'],
						'NEED_DONE_ACTION_TEXT'=>$ESC_POST['NEED_DONE_ACTION_TEXT'],
						'GARDEFOU'=>"rien");
	$msg=create_pack($sql_details,$info_details);
	if ($ESC_POST['REDISTRIB_USE'] == 1){
		$timestamp_redistrib= time();
		$server_dir=$ESC_POST['download_rep_creat'];
		//création du fichier zip pour les serveurs de redistribution
		require_once("libraries/zip.lib.php");
		$zipfile = new zipfile();
		$rep = $ESC_POST['document_root'].$sql_details['timestamp']."/";
		@mkdir($server_dir);
		...
		$digest=crypt_file($server_dir.$timestamp_redistrib."/".$timestamp_redistrib."_redistrib.zip",$ESC_POST["digest_algo"],$ESC_POST["digest_encod"]);
		...
		$sql_details=array('document_root'=>$server_dir,
					   ...
					   'nbfrags'=>$ESC_POST['nbfrags_redistrib'],
					   'name'=>$ESC_POST['NAME'].'_redistrib',
					   'os'=>$ESC_POST['OS'],
					   'description'=>'[PACK REDISTRIBUTION '.$ESC_POST['timestamp'].']',
					   'size'=>$fSize);
					   
		$info_details=array('PRI'=>$ESC_POST['REDISTRIB_PRIORITY'],
						'ACT'=>'STORE',
						'DIGEST'=>$digest,
						'PROTO'=>$ESC_POST['PROTOCOLE'],
						'DIGEST_ALGO'=>$ESC_POST["digest_algo"],
						'DIGEST_ENCODE'=>$ESC_POST["digest_encod"],
						'PATH'=>$ESC_POST['download_server_docroot'],//aller chercher en base le répertoire de stockage des fichiers
						'NAME'=>'',
						'COMMAND'=>'',
						'NOTIFY_USER'=>'0',
						'NOTIFY_TEXT'=>'',
	...
	unset($ESC_POST,$_SESSION['DATA_CACHE']);
	echo $msg;
}
...
if (isset($ESC_POST['valid'])){
	looking4config();
	//vérification de l'existance du fichier
	$fSize = @filesize( $_FILES["teledeploy_file"]["tmp_name"]);
	if( $fSize <= 0 and $ESC_POST['ACTION'] != 'EXECUTE') 
		$error=$l->g(436)." ".$_FILES["teledeploy_file"]["tmp_name"];

	
	//vérification de doublon du nom
	$verifN = "SELECT fileid FROM download_available WHERE name='".$ESC_POST["NAME"]."'";
	...
	//encryptage du fichier
	$digest=crypt_file($_FILES["teledeploy_file"]["tmp_name"],$ESC_POST["digest_algo"],$ESC_POST["digest_encod"]);
	//création du fichier temporaire
	creat_temp_file($ESC_POST['document_root'].$ESC_POST['timestamp'],$_FILES["teledeploy_file"]["tmp_name"]);

	$digName = $ESC_POST["digest_algo"]. " / ".$ESC_POST["digest_encod"];
	
	$title_creat="<tr height='30px'><td colspan='10' align='center'><b>".$l->g(435)."[".$ESC_POST['NAME']."]</b></td></tr>";
	$name_file=$lign_begin.$l->g(446).$td_colspan2.$_FILES["teledeploy_file"]["name"].$lign_end;
	$ident=$lign_begin.$l->g(460).$td_colspan2.$ESC_POST['timestamp'].$lign_end;
...
if (!$ESC_POST){
	...
	$ESC_POST['document_root']=$document_root."/download/";
}
//on garde en hidden le répertoire ou sont créés les paquets de télédéploiement
echo "<input type='hidden' name='document_root' value='".$ESC_POST['document_root']."'>	  
	 <input type='hidden' id='timestamp' name='timestamp' value='".$ESC_POST['timestamp']."'>";
...
$nom= $lign_begin.$l->g(49).$td_colspan2.show_modif($ESC_POST['NAME'],'NAME',0,'',$config_input).$lign_end;
$descr=$lign_begin.$l->g(53).$td_colspan2.show_modif($ESC_POST['DESCRIPTION'],'DESCRIPTION',1).$lign_end;
...
<div id='LAUNCH_div' style='display:none'>".$l->g(446).": </div>".show_modif($ESC_POST['ACTION_INPUT'],'ACTION_INPUT',0,'').$lign_end;
...
//	$default['DOWNLOAD_PRIORITY'] = "5";
	if (!$ESC_POST['REDISTRIB_REP'])
	$ESC_POST['REDISTRIB_REP']=$default['DOWNLOAD_REP_CREAT'];
	if (!$ESC_POST['REDISTRIB_PRIORITY'])
	$ESC_POST['REDISTRIB_PRIORITY']=$default['DOWNLOAD_PRIORITY'];
	...
	echo "<tr><td colspan='3' align=center><div id='REDISTRIB_USE_div' style='display:".($ESC_POST["REDISTRIB_USE"] == 1 ? " block" : "none")."'>";
	echo $sous_tab_beg;
...
echo "<table BGCOLOR='#C7D9F5' BORDER='0' WIDTH = '600px' ALIGN = 'Center' CELLPADDING='0' BORDERCOLOR='#9894B5' >";
	echo $title_user.$notify_user;
		...
		$notify_txt=$lign_begin.$l->g(449).$td_colspan2.show_modif($ESC_POST['NOTIFY_TEXT'],'NOTIFY_TEXT',1).$lign_end;
		$notify_count_down=$lign_begin.$l->g(450).$td_colspan2.show_modif($ESC_POST['NOTIFY_COUNTDOWN'],'NOTIFY_COUNTDOWN',0,'',array('MAXLENGTH'=>4,'SIZE'=>4)).$l->g(511).$lign_end;
		...
		echo "<tr><td colspan='3' align=center><div id='NOTIFY_USER_div' style='display:".($ESC_POST["NOTIFY_USER"] == 1 ? " block" : "none")."'>";
		echo $sous_tab_beg;
	...
	echo $need_done_action;
		...
		$need_done_action_txt=$lign_begin.$l->g(449).$td_colspan2.show_modif($ESC_POST['NEED_DONE_ACTION_TEXT'],'NEED_DONE_ACTION_TEXT',1).$lign_end;
		echo "<tr><td colspan='3' align=center><div id='NEED_DONE_ACTION_div' style='display:".($ESC_POST["NEED_DONE_ACTION"] == 1 ? " block" : "none")."'>";
		echo $sous_tab_beg;
