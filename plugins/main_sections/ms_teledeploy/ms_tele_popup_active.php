...
if ($protectedPost['choix_activ'] == "MAN" and $protectedPost['valid']){	
	//on v�rifie l'existance du fichier info sur le serveur d�sign�
	//pour �tre s�r que la valeur du https soit correcte
...
echo ">";
	...
	//recherche des valeurs par d�faut si les valeurs n'existent pas.
	if (!isset($protectedPost['HTTPS_SERV']) or (!isset($protectedPost['FILE_SERV']) and $protectedPost['choix_activ'] == "MAN")){
		...
		$resdefaultvalues = mysql_query( $reqdefaultvalues, $_SESSION['OCS']["readServer"] );
	...
	elseif($protectedPost['choix_activ'] == "AUTO"){
		...
		$resGroupsServers = mysql_query( $reqGroupsServers, $_SESSION['OCS']["readServer"] );
		$nb_group=0;
		...
		//s'il y a plusieurs groupes de serveur, on propose une liste d�roulante
		if ($nb_group > 1)
	...
	//dans les deux cas, si un choix a �t� fait, on demande l'emplacement du fichier INFO
