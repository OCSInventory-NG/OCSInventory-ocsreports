...
if ($protectedPost['onglet'] == 'CAT') {
    ...
    /**
     * Remove all category without installation software to reduce the number of elements on combobox 
     */
...
AND s.ID IS NOT NULL " . $search_count . " group by formatted";
    ...
    if (!empty($list_cat)) {
        ...
        /**
         * List only softwares that have installation, to reduce the number of itens listed
         */
...
if ($protectedPost['onglet'] == 'NEW') {
    /**
     * List alpha from NEW softwares,that is not classified in any category, but just the installed in some computer, to reduce the time to query and rendering page
     */
...
and cache.ID not in (select sn.ID FROM dico_ignored di 
    ...
    /**
     * List NEW software by alpha, but just software that have installation
     */
...
INNER JOIN software_name sn ON (sn.NAME=di.EXTRACTED)) " . $search_cache;
    ...
    /**
     * Add columns to show more information about the software and FIX search integration
     */
    ...
    /**
     * to bether performance of list of NEW software, but just the installed, not installed no need to be showed 
     */
    $querydico = substr($querydico, 0, -1);
...
if (isset($querydico)) {
    ...
    $tab_options['LIEN_CHAMP']['QTE'] = 'SRCHSOFT'; // FIX multi search integration
...
/**
 * List only categories that have software installed in some computer
 */
