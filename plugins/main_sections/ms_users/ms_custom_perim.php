...
if( $protectedPost['ADD_TAG'] != "" ) {
	...
	@mysql_query( "INSERT INTO tags(tag,login) VALUES('".$tbi."','".$protectedGet["id"]."')", $_SESSION['OCS']["writeServer"]  );
}
...
if(isset($protectedPost['SUP_PROF'])) {
	...
	@mysql_query( "DELETE FROM tags WHERE tag='".$protectedPost['SUP_PROF']."' AND login='".$protectedGet["id"]."'", $_SESSION['OCS']["writeServer"]  );
}
echo "<br><form name='".$form_name."' id='".$form_name."' method='POST'>";
$reqTags ="select tag from tags where login='".$protectedGet['id']."'";
$resTags = mysql_query( $reqTags, $_SESSION['OCS']["readServer"] );
