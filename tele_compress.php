...
header("Content-Disposition: attachment; filename=".$ESC_GET["timestamp"].".zip");
if(isset($ESC_GET["timestamp"])){
	require_once("libraries/zip.lib.php");
	$zipfile = new zipfile();
	//looking for the directory for pack
	if ($ESC_GET['type'] == "server")
	...
	$rep = $document_root.$ESC_GET["timestamp"]."/";
	//echo $rep;
	$dir = opendir($rep);
	while($f = readdir($dir))
	   if(is_file($rep.$f))
