...
if($ESC_POST['Valid_modif_x']){
	$result=creat_group ($ESC_POST['NAME'],$ESC_POST['DESCR'],'','','STATIC');
	if ($result['RESULT'] == "OK"){
		$color="green";
		unset($ESC_POST['add_static_group']);
	}else
	$color="red";
	$msg=$result['RESULT'];	
...
if (isset($ESC_POST['CONFIRM_CHECK']) and  $ESC_POST['CONFIRM_CHECK'] != "")
	$result=group_4_all($ESC_POST['CONFIRM_CHECK']);

//if delete group
if ($ESC_POST['SUP_PROF'] != ""){
	$result=delete_group($ESC_POST['SUP_PROF']);	
...
if (isset($ESC_POST['add_static_group']) and $_SESSION["lvluser"]==SADMIN){
	...
	$img_modif="";
		...
		$name=show_modif($ESC_POST['NAME'],'NAME',0);
		$description=show_modif($ESC_POST['DESCR'],'DESCR',1);
		//show new bottons
