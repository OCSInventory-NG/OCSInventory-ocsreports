...
if (isset($ESC_POST['systemid'])){
	$resInt = mysql_query("SELECT ipaddress FROM networks WHERE hardware_id=".$ESC_POST['systemid'],$_SESSION["readServer"] );
	while( $valInt = mysql_fetch_array( $resInt )){
		if( ! ($res = @mysql_query( "SELECT ipsubnet FROM networks WHERE ipaddress='".$valInt["ipaddress"]."' AND hardware_id='".$ESC_POST["systemid"]."'", $_SESSION["readServer"] ))) {
