...
if (isset($_SESSION['ID_REQ']) and $ESC_POST['origine'] != "machine" and $ESC_POST['origine'] != "group")
$ESC_GET["listid"]=implode(',',$_SESSION['ID_REQ']);


if ($ESC_POST['onglet'] == "" or !isset($ESC_POST['onglet']))
$ESC_POST['onglet'] = $l->g(499);
...
/*	//echo "totot";
	if (!isset($ESC_POST['origine']) and !isset($ESC_GET["listid"])){
		$list_hardware_id="";
	...
	if(isset($ESC_GET["listid"])){
		if ($ESC_POST['CHOISE'] == "REQ"){
			$list_hardware_id=$ESC_GET["listid"];
		}elseif ($ESC_POST['CHOISE'] == "SEL"){
			$list_hardware_id=$ESC_GET["idchecked"];
		}
		$tab_hadware_id=explode(",",$list_hardware_id);
		$add_lbl=" (".count($tab_hadware_id)." ".$l->g(652).")";	
	}elseif(($ESC_POST['origine'] == "machine" or $ESC_POST['origine'] == "group") and $ESC_POST['systemid'] != ""){
		
		$list_hardware_id=$ESC_POST['systemid'];
	}
...
if ($ESC_POST['origine'] == "machine"){
$direction=	"machine.php?option=cd_configuration&systemid=".$ESC_POST["systemid"];	
}elseif ($ESC_POST['origine'] == "group")
$direction=	"index.php?".PAG_INDEX."=".$pages_refs['group_show']."&popup=1&systemid=".$ESC_POST["systemid"];
else
...
//not a sql query
if (isset($ESC_POST['origine'])){
	//looking for value of systemid
	$sql_value_idhardware="select * from devices where name != 'DOWNLOAD' and hardware_id=".$ESC_POST["systemid"];
...
if (isset($ESC_POST['origine'])){
echo "<input type='hidden' id='systemid' name='systemid' value='".$ESC_POST['systemid']."'>";
	echo "<input type='hidden' id='origine' name='origine' value='".$ESC_POST['origine']."'>";
} 
echo "</td></tr></table>";
 echo "</form>";
?>
