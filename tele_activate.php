...
if ($ESC_POST['del_check'] != ''){
	 foreach (explode(",", $ESC_POST['del_check']) as $key){
	 	del_pack($key);
	 	//regénération du cache
		$tab_options['CACHE']='RESET';	 	
	 }	
...
if (!$ESC_POST['SHOW_SELECT']){
$ESC_POST['SHOW_SELECT']='download';
...
if ($ESC_POST['SHOW_SELECT'] == 'download'){
...
if (!isset($document_root)){
		...
		if ($ESC_POST['SHOW_SELECT'] == "server")
...
$tab_options['TYPE']['ZIP']=$ESC_POST['SHOW_SELECT'];
...
if ($ESC_POST['SHOW_SELECT'] == 'download'){
	...
	$activ_manuel=show_modif($ESC_POST['manualActive'],'manualActive',0,'',$config_input);
