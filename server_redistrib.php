...
if($ESC_POST["supp"]){
	if ($ESC_POST["supp"] != "ALL"){
		$verif[0]['sql']="select fileid from download_enable,devices
				where download_enable.id=devices.ivalue
				and download_enable.SERVER_ID=".$ESC_POST["supp"];
		$verif[0]['condition']='EXIST';
		$verif[0]['MSG_ERROR']=$l->g(689)." ".$l->g(687);
		$ok=verification($verif);
		if (isset($ok)){
            mysql_query("delete from download_enable where SERVER_ID=".$ESC_POST["supp"], $_SESSION["writeServer"]) or die(mysql_error($_SESSION["writeServer"]));
			mysql_query("delete from download_servers where hardware_id=".$ESC_POST["supp"], $_SESSION["writeServer"]) or die(mysql_error($_SESSION["writeServer"]));
		}
	}
	elseif ($ESC_POST["supp"] == "ALL"){
		$verif[0]['sql']="select fileid from download_enable,devices
				where download_enable.id=devices.ivalue
...
if (isset($ESC_POST['Valid_modif_x']) and isset($ESC_POST['modif']) and $ESC_POST['modif'] != ""){
	$default_values=look_default_values();
	if (trim($ESC_POST['URL']) == "")
	$ESC_POST['URL']=$default_values['tvalue']['DOWNLOAD_SERVER_URI'];
	if (trim($ESC_POST['REP_STORE']) == "")
	$ESC_POST['REP_STORE']=$default_values['tvalue']['DOWNLOAD_SERVER_DOCROOT'];
		
	if ($ESC_POST['modif'] != "ALL")
	{
		 
			$sql= "update download_servers set URL='".$ESC_POST['URL']."' ,ADD_REP='".$ESC_POST['REP_STORE']."' where hardware_id=".$ESC_POST['modif'];
			mysql_query($sql, $_SESSION["writeServer"]) or die(mysql_error($_SESSION["writeServer"]));
			$sql= "update download_enable set pack_loc='".$ESC_POST['URL']."' where SERVER_ID=".$ESC_POST['modif'];
			mysql_query($sql, $_SESSION["writeServer"]) or die(mysql_error($_SESSION["writeServer"]));
	}else
	{				
			$sql="update download_servers set URL='".$ESC_POST['URL']."' ,ADD_REP='".$ESC_POST['REP_STORE']."' where GROUP_ID=".$systemid;
			mysql_query($sql, $_SESSION["writeServer"]) or die(mysql_error($_SESSION["writeServer"]));
			$sql= "update download_enable set pack_loc='".$ESC_POST['URL']."' where GROUP_ID=".$systemid;
			mysql_query($sql, $_SESSION["writeServer"]) or die(mysql_error($_SESSION["writeServer"]));
	}
}
...
if (isset($systemid))
{
	if ($ESC_POST['tri2'] == "")
	$ESC_POST['tri2']=1;
	if (!(isset($ESC_POST["pcparpage"])) and isset($ESC_GET['res_pag']))
	$ESC_POST["pcparpage"]=$ESC_GET['res_pag'];
	if (!(isset($ESC_POST["page"])) and isset($ESC_GET['page']))
	$ESC_POST["page"]=$ESC_GET['page'];
	$form_name='nb_4_pag';
	...
	$sql="select download_servers.HARDWARE_ID ID,
		...
		where download_servers.GROUP_ID=".$systemid." order by ".$ESC_POST['tri2']." ".$ESC_POST['sens'];
	...
	$i=0;
	//" du groupe ".$data[$ESC_GET['viewmach']]['ID'].
	...
	if ($ESC_POST['modif']!=""  and !isset($ESC_POST['Valid_modif_x']) and !isset($ESC_POST['Reset_modif_x']))
	{
		$tab_name[1]=$l->g(646).": ";
		$tab_name[2]=$l->g(648).": ";
		$tab_typ_champ[1]['DEFAULT_VALUE']=substr($data2[$ESC_POST['modif']]['URL'],7);
		...
		$tab_typ_champ[2]['DEFAULT_VALUE']=$data2[$ESC_POST['modif']]['REP_STORE'];
		$tab_typ_champ[2]['INPUT_NAME']="REP_STORE";
		$tab_typ_champ[2]['INPUT_TYPE']=0;
		$tab_hidden["modif"]=$data2[$ESC_POST['modif']]['ID'];
		...
		if ($ESC_POST['modif'] == "ALL"){
			...
			$title= $l->g(693)." ".$data2[$ESC_POST['modif']]['NAME'];
	        $comment=$l->g(694);
